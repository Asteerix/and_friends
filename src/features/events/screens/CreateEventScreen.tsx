...
export default function CreateEventScreen() {
  ...
  const { pickImage, uploadImage, isUploading, uploadProgress } = useSupabaseStorage();
  ...
  const [coverImage, setCoverImage] = useState<any>(DISCO_IMAGE);
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (coverImageUri) {
        try {
          uploadedImageUrl = await uploadImage(coverImageUri);
        } catch (uploadError) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <Image
            source={coverImageUri ? { uri: coverImageUri } : coverImage}
            style={{
              width: "80%",
              ...
              >
                ...
                <TouchableOpacity
                  ...
                  onPress={async () => {
                    ...
                    if (image) {
                      setCoverImageUri(image.uri);
                      setCoverImage({ uri: image.uri });
                    }
                  }}
                ...
                >
                  {isUploading ? (
                    ...
                    >
                      {coverImageUri ? "Changer l'image" : "Upload Image"}
                    </Text>
                  )}
                </TouchableOpacity>
                {coverImageUri && (
            ...
            {loading ? (
              <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                ...
                <Text style={styles.publishBtnText}>
                  {isUploading ? `Upload... ${Math.round(uploadProgress)}%` : "Publishing..."}
                </Text>
              </View>
            ) : (
