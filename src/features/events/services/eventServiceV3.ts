...
export class EventServiceV3 {
  static async createEvent(eventData: CreateEventData) {
    ...
    try {
      ...
      eventToInsert.extra_data = {
        ...
        // Prix et coûts
        price:
          eventData.costs && eventData.costs.length > 0 && eventData.costs[0]
            ? parseFloat(eventData.costs[0].amount)
            : null,
        currency:
          eventData.costs && eventData.costs.length > 0 && eventData.costs[0]
            ? eventData.costs[0].currency
            : 'EUR',
      ...
      console.log(
        '  - extra_data:',
        eventToInsert.extra_data
          ? 'OUI (' + JSON.stringify(eventToInsert.extra_data).length + ' caractères)'
          : 'NON'
      );
      ...
      Object.entries(extrasResults).forEach(([key, result]) => {
        ...
        console.log(
          `  ${icon} ${key}: ${result.success ? 'Succès' : result.error ? 'Échec' : 'Ignoré'}`
        );
      });
      console.log('');
  ...
  static async uploadCoverImage(imageUri: string, userId: string): Promise<string> {
    ...
    try {
      ...
      const { error } = await supabase.storage.from(bucketName).upload(fileName, blob, {
  ...
  static async addCoHosts(eventId: string, coHosts: any[], invitedBy: string) {
    ...
    const { error } = await supabase.from('event_participants').insert(participantsToAdd);
  ...
  static async uploadEventPhotos(eventId: string, photos: string[], userId: string) {
    ...
    for (const [index, photoUri] of photos.entries()) {
      try {
        ...
        const { error } = await supabase.storage.from('events').upload(fileName, blob, {
          contentType: 'image/jpeg',
          upsert: true,
        });
  ...
  static async addCoverStickers(eventId: string, stickers: any[]) {
    ...
    const { error } = await supabase.from('event_cover_stickers').insert(stickersToAdd);
