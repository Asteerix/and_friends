    ...
    <TouchableOpacity onPress={onPress} activeOpacity={0.85} style={styles.container}>
      <View style={styles.thumbnailContainer}>
        {eventImage && eventImage.hasImage ? (
          eventImage.source ? (
            <Image source={eventImage.source} style={styles.thumbnail} resizeMode="cover" />
          ) : (
            <Image source={{ uri: eventImage.uri }} style={styles.thumbnail} resizeMode="cover" />
          )
        ) : (
          <Image
            source={{
              uri: thumbnail || 'https://via.placeholder.com/400x400/f0f0f0/666666?text=Event',
            }}
        ...
        {(category || event?.event_category) && (
          <View style={styles.categoryBadge}>
            <Text style={styles.categoryIcon}>
              {getCategoryIcon(category || event?.event_category)}
            </Text>
          </View>
        )}
      </View>
