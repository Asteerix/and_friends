...
const latLonToPixel = (lat: number, lon: number, region: any): { x: number; y: number } => {
  const x = (lon - region.longitude + region.longitudeDelta / 2) / region.longitudeDelta * SCREEN_WIDTH;
  const y = (region.latitude + region.latitudeDelta / 2 - lat) / region.latitudeDelta * SCREEN_HEIGHT;
  return { x, y };
};
