...
export default function MemoriesScreen() {
  ...
  useEffect(() => {
    fetchAllMemories();
    // Disable real-time subscription to avoid UI reloads
    // const unsubscribe = subscribeToAllStories();
    // return unsubscribe;
  }, []);
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const shareOptions = {
        ...
        title: `Memory by @${currentMemory.user?.username || 'Unknown'}`,
      };
    ...
    <View style={styles.container}>
      ...
      {showReportModal && memories[currentIndex] && (
        <ReportModal
          ...
          targetName={`Memory de ${memories[currentIndex].user?.username || 'User'}`}
        />
      )}
    </View>
  );
