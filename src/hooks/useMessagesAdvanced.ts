...
export function useMessagesAdvanced(chatId?: string) {
  ...
  // Send a message
  const sendMessage = async (content: string, messageType: "text" | "image" | "file" = "text", metadata?: Record<string, any>) => {
    if (!session?.user || !chatId || !content.trim()) return { error: { message: "Invalid parameters" } };
  ...
  // Create a new chat
  const createChat = async (name: string, isGroup: boolean, participantIds: string[], eventId?: string) => {
