...
export const useRatings = () => {
  ...
  const getUserRatingStats = useCallback(async (userId: string): Promise<RatingStats | null> => {
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_user_rating_stats', {
        p_user_id: userId,
      });
  ...
  const getUserGivenRatings = useCallback(
    async (userId: string, limit = 50, offset = 0): Promise<Rating[]> => {
      try {
        setLoading(true);
        setError(null);

        const { data, error: rpcError } = await supabase.rpc('get_user_given_ratings', {
          p_user_id: userId,
          p_limit: limit,
          p_offset: offset,
        });
  ...
  const getUserReceivedRatings = useCallback(
    async (userId: string, limit = 50, offset = 0): Promise<Rating[]> => {
      try {
        setLoading(true);
        setError(null);

        const { data, error: rpcError } = await supabase.rpc('get_user_received_ratings', {
          p_user_id: userId,
          p_limit: limit,
          p_offset: offset,
        });
  ...
  const upsertRating = useCallback(
    async (
      ...
      try {
        ...
        const { error: rpcError } = await supabase.rpc('upsert_rating', {
  ...
  const deleteRating = useCallback(async (ratingId: string): Promise<boolean> => {
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('delete_rating', {
  ...
  const canRateUser = useCallback(
    async (toUserId: string, eventId?: string): Promise<boolean> => {
      try {
        if (!profile?.id) return false;

        const { data, error: rpcError } = await supabase.rpc('can_rate_user', {
          p_from_user_id: profile.id,
          p_to_user_id: toUserId,
          p_event_id: eventId || null,
        });
  ...
  const getEventParticipantRatings = useCallback(async (eventId: string) => {
    try {
      ...
      const { data, error: rpcError } = await supabase.rpc('get_event_participant_ratings', {
        p_event_id: eventId,
      });
