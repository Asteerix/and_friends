...
export function useSecureSupabase() {
  ...
  const secureUpdateProfile = useCallback(async (
    userId: string,
  ...
  const secureGetEvents = useCallback(async (
    filters?: {
      userId?: string;
      isPrivate?: boolean;
      fromDate?: string;
  ...
  const secureAddFriend = useCallback(async (
    userId: string,
    friendId: string
  ): Promise<SafeResult<any>> => {
    // Validate UUIDs
  ...
  const secureCreateNotification = useCallback(async (
  ...
  const secureSearchUsers = useCallback(async (
    searchTerm: string,
    limit: number = 10
  ): Promise<SafeResult<any[]>> => {
  ...
  const secureBatchOperation = useCallback(async <T>(
    ...
    try {
      const results = await Promise.all(
        operations.map(op => safeSupabaseOperation(op))
      );
