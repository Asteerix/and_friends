...
export const MemoriesProvider: React.FC<MemoriesProviderProps> = ({ children }) => {
  ...
  const toggleLike = useCallback(
    async (storyId: string) => {
      ...
      try {
        ...
        setStories((prev) =>
          prev.map((s) =>
            s.id === storyId ? { ...s, is_liked: isLiked, likes_count: story.likes_count } : s
          )
        );
    ...
    async (storyId: string) => {
        ...
        prev.map((s) =>
          s.id === storyId
            ? {
                ...s,
                is_saved: !isSaved,
                saves_count: isSaved ? s.saves_count - 1 : s.saves_count + 1,
              }
            : s
        )
      );
      ...
      try {
        if (isSaved) {
          ...
          const { error } = await supabase.from('story_saves').insert({
            story_id: storyId,
            user_id: session.user.id,
          });
        ...
        setStories((prev) =>
          prev.map((s) =>
            s.id === storyId ? { ...s, is_saved: isSaved, saves_count: story.saves_count } : s
          )
        );
  ...
  const addView = useCallback(
    async (storyId: string) => {
      ...
      try {
        ...
        setStories((prev) =>
          prev.map((s) =>
            s.id === storyId ? { ...s, has_viewed: false, views_count: story.views_count } : s
          )
        );
  ...
  const addReply = useCallback(
    async (storyId: string, text: string, parentReplyId?: string) => {
      if (!session?.user) return;
      ...
      try {
        ...
        if (data) {
          ...
          setStories((prev) =>
            prev.map((s) => (s.id === storyId ? { ...s, replies_count: s.replies_count + 1 } : s))
          );
        }
      } catch (err) {
    ...
    async (replyId: string) => {
      ...
      try {
        ...
        if (storyId) {
          setStories((prev) =>
            prev.map((s) =>
              s.id === storyId ? { ...s, replies_count: Math.max(0, s.replies_count - 1) } : s
            )
          );
        }
      } catch (err) {
