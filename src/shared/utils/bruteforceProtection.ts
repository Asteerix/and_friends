...
export async function recordFailedOTPAttempt(phoneNumber: string): Promise<BanStatus> {
  try {
    ...
    const { data, error } = await supabase.rpc('record_failed_otp_attempt', {
      ...
      if (result.is_banned) {
        const banStatus: BanStatus = {
          ...
          timeRemainingSeconds: Math.max(
            0,
            Math.floor((new Date(result.banned_until).getTime() - Date.now()) / 1000)
          ),
        };
...
export async function checkBanStatus(phoneNumber: string): Promise<BanStatus> {
  try {
    ...
    // Check with Supabase first (server is source of truth)
    // Only fall back to local if server fails
    const { data, error } = await supabase.rpc('check_otp_ban_status', {
      phone_num: phoneNumber,
      device_id_param: deviceId,
    });
    ...
    if (error) {
      ...
      // Fall back to local ban status if available
      ...
      if (localBan.isBanned && localBan.phoneNumber === phoneNumber) {
        if (localBan.bannedUntil && new Date(localBan.bannedUntil) > new Date()) {
          return {
            ...
            timeRemainingSeconds: Math.max(
              0,
              Math.floor((new Date(localBan.bannedUntil).getTime() - Date.now()) / 1000)
            ),
          };
        }
      }
    ...
    if (data && data.length > 0) {
      ...
      if (result.is_banned) {
        const banStatus: BanStatus = {
          ...
          reason: result.ban_reason,
          phoneNumber: phoneNumber,
          timeRemainingSeconds: Math.max(0, result.time_remaining_seconds || 0),
        };
...
async function recordFailedAttemptLocally(phoneNumber: string): Promise<BanStatus> {
  try {
    ...
    const attempts: FailedAttempt[] = attemptsJson ? JSON.parse(attemptsJson) : [];
    ...
    if (banJson) {
      ...
      if (ban.bannedUntil && new Date(ban.bannedUntil) > new Date()) {
        return {
          ...
          timeRemainingSeconds: Math.max(
            0,
            Math.floor((new Date(ban.bannedUntil).getTime() - Date.now()) / 1000)
          ),
        };
      }
    }
