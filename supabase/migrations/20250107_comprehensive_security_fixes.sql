...
-- Update get_friends function (with correct return type)
...
-- Update get_event_messages function  
...
-- Update get_friend_requests function (with correct return type)
CREATE OR REPLACE FUNCTION public.get_friend_requests(p_user_id UUID, p_type TEXT DEFAULT 'received')
RETURNS TABLE(
  id UUID,
  username TEXT,
  full_name TEXT,
...
-- Update search_users function (with correct parameters and return type)
...
BEGIN
  IF EXISTS (
    ...
    -- Create a policy to allow read access for authenticated users
