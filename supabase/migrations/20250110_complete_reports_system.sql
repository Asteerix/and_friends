...
-- Migrate existing data from old reports table if it exists
DO $$ 
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'reports_old') THEN
    INSERT INTO reports (
      id,
      reporter_id,
      reported_type,
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_unique_report ON reports(reporter_id, reported_type, reported_id);
...
CREATE INDEX IF NOT EXISTS idx_report_actions_created ON report_actions(created_at DESC);
...
DROP TRIGGER IF EXISTS auto_hide_reported_content ON reports;
...
DROP POLICY IF EXISTS "Admins can create report actions" ON report_actions;
